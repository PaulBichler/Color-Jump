...
SceneNode::Ptr LevelLoader::LoadLevelLayer(LevelInfo& level_info, const std::string& csv_path, const bool is_collider_layer)
{
	...
	for (int row = 0; row < m_level_data_vector.size(); row++)
	{
		for (int col = 0; col < m_level_data_vector[row].size(); col++)
		{
			...
			{
				//Create player 1
				std::unique_ptr<Character> player_1(m_tile_factory.CreatePlayer(tile_type, ECharacterType::kRed, spawn_pos));
			...
			{
				//Create player 2
				std::unique_ptr<Character> player_2(m_tile_factory.CreatePlayer(tile_type, ECharacterType::kBlue, spawn_pos));
			...
			case kFinishPlatformPart:
				...
				CreatePlatform(tile_type, level_info, row, col, level_parent, spawn_pos);
				break;
			case kNone:
...
void LevelLoader::CreatePlatform(const ETileType tile_type, LevelInfo& level_info,
