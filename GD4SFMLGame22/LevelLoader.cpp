                                           ...
                                           const bool is_collider_layer)
{
	...
	for (int row = 0; row < m_level_data_vector.size(); row++)
	{
		for (int col = 0; col < m_level_data_vector[row].size(); col++)
		{
			...
			{
				std::unique_ptr<Character> player_1(
					m_tile_factory.CreatePlayer(id, tile_type, ECharacterType::kRed, spawn_pos));
				level_info.player_1 = player_1.get();
				level_parent->AttachChild(std::move(player_1));
			}
			else if (tile_type == kBluePlayer)
			{
				std::unique_ptr<Character> player_2(
					m_tile_factory.CreatePlayer(id, tile_type, ECharacterType::kBlue, spawn_pos));
...
void LevelLoader::CreatePlatform(const EPlatformType type, const ETileType tile_type, LevelInfo& level_info,
...
void LevelLoader::AddPlatformParts(Platform* platform, const int row, const int col, const SceneNode::Ptr& parent,
                                   const ETileType tile_type, sf::Vector2f spawn_pos)
{
	const ETileType type = static_cast<ETileType>(m_level_data_vector[row][col]);

	std::unique_ptr<PlatformPart>
		platform_part(m_tile_factory.CreatePlatformPart(type, spawn_pos, platform, tile_type));
	...
	for (size_t i = row + 1; i < m_level_data_vector.size(); i++)
	{
		...
		spawn_pos.y += m_level_data.m_tile_size.y;
		std::unique_ptr<PlatformPart> vertical_platform_part(
			m_tile_factory.CreatePlatformPart(type, spawn_pos, platform, tile_type));
	...
	for (size_t j = col + 1; j < m_level_data_vector[row].size(); j++)
	{
		...
		spawn_pos.x += m_level_data.m_tile_size.x;
		std::unique_ptr<PlatformPart> horizontal_platform_part(
			m_tile_factory.CreatePlatformPart(type, spawn_pos, platform, tile_type));
